...
#################################################################
x_2 = tf.nn.relu(conv2d(x_1, W_1)+b_1);

b_2 = bias([64]);
W_2 = weight([3,3,32,64]);

#################################################################
x_3 = tf.nn.relu(conv2d(x_2, W_2)+b_2);

b_3 = bias([128]);
W_3 = weight([3,3,64,128]);

#################################################################
x_4 = tf.nn.relu(conv2d(x_3, W_3)+b_3);
...
y_pred = tf.nn.sigmoid(out_fc)*2-1;
loss = tf.nn.l2_loss(y_pred-y_train);
for w in reg:
...
if __name__ == "__main__":
	...
	board_batch, label_batch = read_dataset(filenames=[TFRECORD_FOLDER+str(i)+".tfrecord" for i in range(10)]);

	with tf.Session() as sess:
		sess.run(tf.global_variables_initializer());
		for i in range(50):
			train_step.run(feed_dict={x:board_batch, y_train:label_batch, dropout_keep_prob:hp.Model["DROP_OUT_KEEP_PROB"]});
			print loss.eval();
